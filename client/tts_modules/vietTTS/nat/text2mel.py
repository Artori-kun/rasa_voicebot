...
def predict_duration(tokens, duration_dic):
    ...
    #     duration_dic = pickle.load(f)
    x = DurationInput(
        np.array(tokens, dtype=np.int32)[None, :],
        np.array([len(tokens)], dtype=np.int32),
        None
    )
    return forward_fn(duration_dic['params'], duration_dic['aux'], duration_dic['rng'], x)[0]


def text2tokens(text, phonemes, lexicon):
...
def predict_mel(tokens, durations, acoustic_dic):
    # ckpt_fn = FLAGS.ckpt_dir / 'acoustic_ckpt_latest.pickle'
    # with open(ckpt_fn, 'rb') as f:
    #     acoustic_dic = pickle.load(f)
    last_step, params, aux, rng, optim_state = acoustic_dic['step'], acoustic_dic['params'], acoustic_dic['aux'], \
                                               acoustic_dic['rng'], acoustic_dic['optim_state']
...
def text2mel(text,
             ...
             acoustic_dic,
             silence_duration):
    tokens = text2tokens(text, phonemes, lexicon)
    durations = predict_duration(tokens, duration_dic)
    durations = jnp.where(
    ...
    mels = predict_mel(tokens, durations, acoustic_dic)
